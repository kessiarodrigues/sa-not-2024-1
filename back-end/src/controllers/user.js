...
controller.create = async function (req, res) {
  try {
    ...
    // Somente usuários do nível administrador podem criar novos usuários
    // Caso contrário, retorna HTTP 403: Forbidden

    //API5:2023 – Falha de autenticação a nível de função.
    //Caso o usuário tenha uma alteração de função e por ////alguma inconsistencia de dados
    //continue com a coluna is_admin = true
    //ele ainda terá esse acesso sem permissão
    //ou então caso seja um usuário desligado, já que o
    //banco de dados não tem um soft_delete
    //caso não seja removida a linha deste usuário
    //ele poderá ter permissões de algo que não deveria
    //deveria ser validado também se o usuário é ativo, e seu nivel de acesso
...
function getUserLoginParams(user) {
  ...
  const canLoginAfter = lastLogin
    ? addMinutes(lastLogin, currentDelayMinutes)
    : new Date(1900, 1, 1);
...
controller.login = async function (req, res) {
  try {
    ...
    console.log(
      `Tentativas: ${currentLoginAttempts}, pode tentar após ${format(canLoginAfter, 'PPpp')}`
    );
    ...
    if (
      user.login_attempts >= Number(process.env.MAX_LOGIN_ATTEMPTS) &&
